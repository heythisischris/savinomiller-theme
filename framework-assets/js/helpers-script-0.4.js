...
var javascriptResizeFunctionArray = [];
...
function executeOnResize(myFunction) {
    javascriptResizeFunctionArray.push(myFunction);
}
...
$(document).ready(function(){
    ...
    executeOnResize('sampleFunction');
    setPrettyUi();
    setNavigationTrail();
});
...
function javascriptBeacon() {
    ...
    if(lastRecordedWidth != windowWidth || lastRecordedHeight != windowHeight){

        resizeEvent();
        lastRecordedHeight = windowHeight;
        lastRecordedWidth = windowWidth;
    }
	/*
...
function resizeEvent() {
    myTrace('Resize event windowWidth: '+windowWidth+' windowHeight: '+windowHeight);
    for(i = 0; i < javascriptResizeFunctionArray.length; i++){
        eval(javascriptResizeFunctionArray[i]+'()');
    }
}
